# Raw Table Creation
CREATE OR REPLACE EXTERNAL TABLE `mgmt-467-project-1.bike_raw.weather_daily_ext` (
  date DATE,
...
OPTIONS (
  format = 'CSV',
  uris = ['gs://mgmt467-final-project/weather/raw/open_meteo_daily_monthly/*_us_units.csv'],
  skip_leading_rows = 1
);

# Row Check
SELECT * FROM `mgmt-467-project-1.bike_raw.weather_daily_ext` LIMIT 10;
SELECT MIN(date) AS min_date, MAX(date) AS max_date, COUNT(*) AS row_count
FROM `mgmt-467-project-1.bike_raw.weather_daily_ext`;

# Curated Table Creation
CREATE OR REPLACE TABLE `mgmt-467-project-1.bike_curated.weather_daily`
...
FROM `mgmt-467-project-1.bike_curated.weather_daily`;
