CREATE OR REPLACE TABLE `ProjectIDPlaceholder.bike_curated.weather_forecast_features` AS
SELECT
  forecast_date,
  temperature_2m_max,
  temperature_2m_min,
...
FROM `ProjectIDPlaceholder.bike_raw.weather_forecast_stream`;
...
WITH
  ...
  features AS (
    SELECT
      f.*
    FROM `ProjectIDPlaceholder.bike_curated.weather_forecast_features` f
